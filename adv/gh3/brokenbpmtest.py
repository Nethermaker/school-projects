        ...
        with open(self.chart, 'rb') as infile:
            for line in infile:
                ...
                if 'AudioStream' in line:
                    ...
                    total_y = 720
                    #print zip(note_bpm_list, note_bpm_list[1:])
                    for bpm in zip(note_bpm_list, note_bpm_list[1:]):
                        tick1, tick2, bpm1, bpm2 = bpm[0][0], bpm[1][0], bpm[0][1], bpm[1][1]
                        #print tick1, tick2, bpm1, bpm2
                        tick_difference = tick2 - tick1
                        #print tick_difference
                        tick_beat_length = tick_difference / float(self.resolution)
                        #print tick_beat_length
                        pixels_per_beat = (bpm1 / 60.0) * 360
                        #print pixels_per_beat
                        total_y -= (tick_beat_length * pixels_per_beat) / self.divisor
                    
                    chord = False
                    sustain_tick = int(line[-1]) + tick
                    sustain_y = 0
                    if sustain_tick != tick:
                        sustain_y = 720
                        sustain_bpm_list = [i for i in self.bpm_list if i[0] <= tick + sustain_tick]
                        for bpm in zip(sustain_bpm_list, sustain_bpm_list[1:]):
                            tick1, tick2, bpm1, bpm2 = bpm[0][0], bpm[1][0], bpm[0][1], bpm[1][1]
                            tick_difference = tick2 - tick1
                            tick_beat_length = tick_difference / float(self.resolution)
                            pixels_per_beat = (bpm1 / 60.0) * 360
                            sustain_y -= (tick_beat_length * pixels_per_beat) / self.divisor
...
##                        sustain = int(round(sustain))
                    ...
                    if note_type == 0:
                        color = 'green'
                        self.note_list.append(Note(tick, total_y, color, chord, self.game, total_y, sustain_tick))
                    elif note_type == 1:
                        color = 'red'
                        self.note_list.append(Note(tick, total_y, color, chord, self.game, total_y, sustain_tick))
                    elif note_type == 2:
